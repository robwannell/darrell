...
before_filter :authorize, except: [:index, :show]
  ...
  # GET /pics/1.json
  def show
     @piccie = Pic.find(params[:id])
    @prev_photo = Pic.where('(id < ?)', @piccie).order(:id => 'DESC').first
    @next_photo = Pic.where('(id > ?)', @piccie).order(:id).first
  end
  ...
  private
    ...
    def pic_params
      params.require(:pic).permit(:caption, :avatar)
    end
end
